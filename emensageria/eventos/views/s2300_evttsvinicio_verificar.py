...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        ocorrencias_lista = s2300evtTSVInicioOcorrencias.objects.using( db_slug ).filter(evento_id=s2300_evttsvinicio_id, excluido = False).all()
...
def recibo(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        return render(request, 's2300_evttsvinicio_recibo.html', context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
