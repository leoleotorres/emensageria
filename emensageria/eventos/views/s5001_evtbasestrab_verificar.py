...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        ocorrencias_lista = s5001evtBasesTrabOcorrencias.objects.using( db_slug ).filter(evento_id=s5001_evtbasestrab_id, excluido = False).all()
...
def recibo(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        return render(request, 's5001_evtbasestrab_recibo.html', context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
