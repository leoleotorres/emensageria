...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        ocorrencias_lista = s1020evtTabLotacaoOcorrencias.objects.using( db_slug ).filter(evento_id=s1020_evttablotacao_id, excluido = False).all()
...
def recibo(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        return render(request, 's1020_evttablotacao_recibo.html', context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
