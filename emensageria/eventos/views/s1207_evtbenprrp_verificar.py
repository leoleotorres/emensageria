...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        ocorrencias_lista = s1207evtBenPrRPOcorrencias.objects.using( db_slug ).filter(evento_id=s1207_evtbenprrp_id, excluido = False).all()
...
def recibo(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        return render(request, 's1207_evtbenprrp_recibo.html', context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
