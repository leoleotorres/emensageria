...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        ocorrencias_lista = s2241evtInsApoOcorrencias.objects.using( db_slug ).filter(evento_id=s2241_evtinsapo_id, excluido = False).all()
...
def recibo(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        return render(request, 's2241_evtinsapo_recibo.html', context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
