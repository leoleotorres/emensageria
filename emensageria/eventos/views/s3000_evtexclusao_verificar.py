...
def verificar(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        ocorrencias_lista = s3000evtExclusaoOcorrencias.objects.using( db_slug ).filter(evento_id=s3000_evtexclusao_id, excluido = False).all()
...
def recibo(request, hash, slug=0):
    ...
    if permissao.permite_listar:
        ...
        return render(request, 's3000_evtexclusao_recibo.html', context)
    else:
        context = {
            'usuario': usuario, 
            'conta': conta, 
