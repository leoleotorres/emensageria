...
def salvar(request, hash):
    ...
    if permissao.permite_visualizar:
        ...
        s1298_evtreabreevper_ocorrencias_form = None
        s1298_evtreabreevper_ocorrencias_lista = None
        if s1298_evtreabreevper_id:
            s1298_evtreabreevper = get_object_or_404(s1298evtReabreEvPer.objects.using( db_slug ), excluido = False, id = s1298_evtreabreevper_id)
       
            s1298_evtreabreevper_ocorrencias_form = form_s1298_evtreabreevper_ocorrencias(initial={ 'evento': s1298_evtreabreevper }, slug=db_slug)
            s1298_evtreabreevper_ocorrencias_form.fields['evento'].widget.attrs['readonly'] = True
            s1298_evtreabreevper_ocorrencias_lista = s1298evtReabreEvPerOcorrencias.objects.using( db_slug ).filter(excluido = False, evento_id=s1298_evtreabreevper.id).all()
        else:
